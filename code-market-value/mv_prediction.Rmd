...
r.passing$Total.TotDist[r.passing$Total.TotDist > 3000] <- max(r.passing$Total.TotDist[r.passing$Total.TotDist < 3000])
```
...
```{r}
boxplot(r.passing %>% select(-c(Player, Nation, Squad, League)))
boxplot(passing %>% select(-c(mv)))
```
